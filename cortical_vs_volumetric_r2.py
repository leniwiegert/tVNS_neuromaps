...
def parcellate_mni152_with_atlas(mni152_data, atlas_data):
    ...
    unique_regions = np.unique(atlas_data) # Find unique regions from atlas_data
    parcellated_data = np.zeros_like(atlas_data, dtype=float) # Initialize Parcellated Data Array

    for region in unique_regions:
        if region == 0:  # Skip background
            continue
        mask = atlas_data == region # Create mask to identify locations in atlas_data that correspond to current region
        region_data = mni152_data[mask] # Extract region
        parcellated_data[mask] = np.mean(region_data)  # Calculate the mean value within the region
...
for i, (key, value) in enumerate(corr_values_cortical_single_maps.items()):
    ...
    for category, color in zip(color_categories.values(), colors):
        for j in category:
            y_values = np.array([[value[j]], [subcortical_values[j]]])
            plt.plot(x_values, y_values, marker='o', color=color, label=f'Map {j + 1}')
...
plt.suptitle('Spaghetti Plot of r-values for Single Subject Data', fontsize=16)
...
Traceback (most recent call last):
  File "/home/leni/PycharmProjects/tVNS_neuromaps/cortical_vs_volumetric_r2.py", line 530, in <module>
    y_values = np.array([[value[j]], [subcortical_values[j]]])
