...
nulls_single = alexander_bloch(mean_img_fslr_parc, atlas='fsLR', density='32k', parcellation=parcels_fslr_32k)
...
for source in annotation_sources:
    ...
    for i in range(1, 42):
        ...
        # Parcellate single volumes
        vol_subcort = parcellater.transform(vol_gm, space='MNI152')

        # Parcellate annotation
        #anno_subcort = parcellater.transform(vol_gm, space='MNI152')

        # Calculate spatial correlations
        #corr, pval = stats.compare_images(vol_fslr_parc, anno_fslr_parc, nulls=nulls_single)
        #corr_values_single.append(corr)

        # Append correlation values for the current volume to the list for all volumes
        #all_corr_values_single.append(corr_values_single)

        # Save the correlation values as a NumPy array
        #corr_values_path = os.path.join(data_directory, f'corr_values_{source}_vol_{i}.npy')
        #np.save(corr_values_path, corr_values_single)
...
parc_fslr_vol_2_img = np.load(os.path.join(data_directory,'parc_fslr_vol_2.npy'))
parc_fslr_vol_12_img = np.load(os.path.join(data_directory,'parc_fslr_vol_12.npy'))
parc_fslr_vol_13_img = np.load(os.path.join(data_directory,'parc_fslr_vol_13.npy'))
parc_fslr_vol_20_img = np.load(os.path.join(data_directory,'parc_fslr_vol_20.npy'))
parc_fslr_vol_30_img = np.load(os.path.join(data_directory,'parc_fslr_vol_30.npy'))
parc_fslr_vol_37_img = np.load(os.path.join(data_directory,'parc_fslr_vol_37.npy'))

# Load the brain atlas defining the ROIs
mean_img_subcort_parc = parcellater.transform(mean_img_gm, space='MNI152')
parc_subcort_vol_2_img = np.load(os.path.join(data_directory,'parc_subcort_vol_2.npy'))
parc_subcort_vol_12_img = np.load(os.path.join(data_directory,'parc_subcort_vol_12.npy'))
parc_subcort_vol_13_img = np.load(os.path.join(data_directory,'parc_subcort_vol_13.npy'))
parc_subcort_vol_20_img = np.load(os.path.join(data_directory,'parc_subcort_vol_20.npy'))
parc_subcort_vol_30_img = np.load(os.path.join(data_directory,'parc_subcort_vol_30.npy'))
parc_subcort_vol_37_img = np.load(os.path.join(data_directory,'parc_subcort_vol_37.npy'))

# Plotting the annotations as well:
...
def map_roi_to_color(roi_index):
    # Here, implement logic to map ROI index to subnetwork color based on your mapping
