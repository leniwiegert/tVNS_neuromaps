...
# Save code with pickle

data_before_plotting = pd.DataFrame(all_corr_values_single,  columns=annotation_sources, index=np.arange(1, 11))

pickle_file_path = (f'{data_directory}data_before_plotting.pkl')

# Save to pickle
data_before_plotting.to_pickle(pickle_file_path)
print(f'Pickle data has been saved.')
...
'''
# Version 27/02
# Create a heatmap of the significant p-values
plt.figure(figsize=(10, 6))
sns.heatmap(p_values_only, cmap='coolwarm', annot=True, fmt='.2f', xticklabels=annotation_sources, yticklabels=np.arange(1, 42))
plt.xlabel('Annotation Sources')
plt.ylabel('Volume Number')
plt.title('Significant P-Values (p <= 0.05) for Each Volume and Each Annotation Source')
plt.show()
'''
